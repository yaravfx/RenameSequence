...
class FindSequence:
    ...
    def get_potential_names(self, filename, numbers):
        potential_list = []
        for number in numbers:
            result = {}
            potential_name = self.get_sequence_info(filename, number)
            result[potential_name] = number
            potential_list.append(result)
        self.sequenceList.append(potential_list)

    def compare_frames(self, current_frame, next_frame):
        for current_names in current_frame:
            print current_names
    ...
    def main(self):
        ...
        for filename in files:
            ...
            if not frame_numbers:  # if there's no number, it's a new sequence
                ...
                self.get_potential_names(filename, frame_numbers)

        for index, current_frame in enumerate(self.sequenceList):
            if index + 1 < len(self.sequenceList):
                print current_frame
                next_frame = self.sequenceList[index+1]
                print next_frame
                # compare the one filename and the next
                self.compare_frame_name(current_frame, next_frame)
    ...
    def get_sequences(self):
        ...
        for index, filename in enumerate(files):
            ...
            if index < len(files) - 1:
                ...
                print "first_frame_numbers", first_frame_numbers
                frame_coverage = []
